...
final class TermuxPreferences {
    ...
    boolean toggleShowExtraKeys(Context context) {
        ...
        PreferenceManager.getDefaultSharedPreferences(context).edit().putBoolean(SHOW_EXTRA_KEYS_KEY, mShowExtraKeys).commit();
    ...
    void changeFontSize(Context context, boolean increase) {
        ...
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        prefs.edit().putString(FONTSIZE_KEY, Integer.toString(mFontSize)).commit();
    }
    ...
    void setScreenAlwaysOn(Context context, boolean newValue) {
        ...
        PreferenceManager.getDefaultSharedPreferences(context).edit().putBoolean(SCREEN_ALWAYS_ON_KEY, newValue).commit();
    }
    ...
    void reloadFromProperties(Context context) {
        ...
        try {
            if (propsFile.isFile() && propsFile.canRead()) {
                ...
                props.load(in);//new InputStreamReader(in, "UTF-8"));
        ...
        switch (props.getProperty("bell-character", "beep")) {
            case "beep":
        ...
        mUseDarkUI = props.getProperty("use-black-ui", "true");

        try {
			
            //JSONArray arr = new JSONArray(props.getProperty("extra-keys", "[['ESC', 'TAB', 'CTRL', 'ALT', '-', 'DOWN', 'UP']]"));
			//JSONArray arr = new JSONArray(props.getProperty("extra-keys", "[['UP', 'ESC', 'TAB', 'CTRL' , 'DOWN']]"));
			JSONArray arr = new JSONArray(props.getProperty("extra-keys", "[['ESC', 'TAB', 'CTRL' , 'DOWN', 'UP']]"));
    ...
    private void parseAction(String name, int shortcutAction, Properties props) {
        ...
        if (!(parts.length == 2 && parts[0].trim().equals("ctrl")) || input.equals("") /*input.isEmpty()*/ || input.length() > 2) {
