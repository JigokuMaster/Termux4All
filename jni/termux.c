        ...
        jint columns)
{
    ...
    if(grantpt(ptm) || unlockpt(ptm))
    {
        return throw_runtime_exception(env, "Cannot unlockpt() on /dev/ptmx");
    }

    char devname[64];
    memset(devname, 0, sizeof(devname));
    errno = 0;
    int pts_ret = ptsname_r(ptm, devname, sizeof(devname));
    if(pts_ret && errno)
    {
    ...
    if (pid < 0) {
        ...
        *pProcessId = (int) pid;
        ...
        dup2(pts, 1);
        dup2(pts, 2);
        int properties_fd = find_properties_fd();
        ...
        if (self_dir != NULL) {
            ...
            while ((entry = readdir(self_dir)) != NULL) {
                ...
                if(fd != properties_fd && fd > 2 && fd != self_dir_fd ){
                    close(fd);
                }
